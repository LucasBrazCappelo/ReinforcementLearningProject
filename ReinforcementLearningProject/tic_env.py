...
class TictactoeEnv:
    ...
    def step(
        ...
        if self.grid[position] != 0:
            raise ValueError(f"There is already a chess on position {position}.")
...
class OptimalPlayer:
    ...
    def center(self, grid: np.ndarray) -> List[Tuple[int, int]]:
        """Pick the center if its available, if it's the first step of the game, center or corner
        are all optimal."""
    ...
    def block_fork(self, grid: np.ndarray) -> List[Tuple[int, int]]:
        ...
        If there is only one possible fork from opponent, block it. Otherwise, player should force
        opponent to block win by making two in a row or column Among all possible force win
        positions, choose positions in opponent's fork in prior
        """
        oppon_val: int = -1 if self.player == "X" else 1
    ...
    def act(self, grid: np.ndarray) -> Tuple[int, int]:
        """Goes through a hierarchy of moves, making the best move that is currently available each
        time (with probability 1-self.epsilon).
